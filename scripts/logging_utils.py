...
def get_logger(name: str):
    logger = std_logging.getLogger(name.split(".")[-1])
    mode: str = os.getenv("ENV", "prod").lower()

    logger.setLevel(std_logging.DEBUG if mode != "prod" else std_logging.INFO)
    ...
    logger.debug(f"Logging mode is {std_logging.getLevelName(logger.getEffectiveLevel())}")
    return logger