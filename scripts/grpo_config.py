...
from lrs_lookup import get_grpo_lr, hash_model, get_grpo_python_lr
...
def get_training_json(train_info: dict) -> dict:
    ...
    if train_info["find_lk_lr"]:
        ...
        if not has_python_execution:
            ...
            lr = get_grpo_python_lr(model_name, lr_amplify_rate)
    ...
    # Step 2: Apply regularization ratio
    run_config["learning_rate"] *= train_info["reg_ratio"]
    
    # Step 3: Calculate min/max lr for LR finder based on final learning rate
    # Wider range for better exploration (especially lower bound)
    current_lr = run_config["learning_rate"]
    run_config["min_lr"] = current_lr * train_info["min_lr"]
    run_config["max_lr"] = current_lr * train_info["max_lr"]

    run_cmd = get_run_cmd(run_config, run_config["gpu_nums"])
    train_request["min_lr"] = run_config["min_lr"]
    train_request["max_lr"] = run_config["max_lr"]
    train_request["find_lr"] = train_info.get("find_lr", True)
