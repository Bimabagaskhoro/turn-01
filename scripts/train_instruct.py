...
from callbacks import (
    ...
    DynamicGradientAccumulationCallback
)
...
def main():
    ...
    # training_args.eval_dataset = dev_ds
    ...
    log_info(f"Default/Initial learning_rate: {training_args.learning_rate:.2e}")
    log_info(f"LR Finder Config: min_lr={min_lr:.2e}, max_lr={max_lr:.2e}, find_lr={find_lr}")
    
    if find_lr:
        original_batch_size = training_args.per_device_train_batch_size
        lr_finder_batch_size = max(1, original_batch_size // 4)
        
        log_info(f"Using reduced batch size for LR finder: {original_batch_size} -> {lr_finder_batch_size}")
        ...
        try:
            ...
            log_info(f"Found optimal LR: {effective_lr:.2e}")
        except Exception as e:
            if "cuda" in str(e).lower():
                log_info(f"CUDA error with batch_size={lr_finder_batch_size}: {e}")
                log_info("Retrying LR finder with batch_size=1 as fallback")
                ...
                try:
                    ...
                    log_info(f"Found optimal LR with batch_size=1: {effective_lr:.2e}")
                except Exception as e2:
                    log_info(f"LR finder failed even with batch_size=1: {e2}")
                    effective_lr = training_args.learning_rate
                    best_state_dict = None
                    best_optimizer_state = None
                    log_info(f"Using default LR: {effective_lr:.2e}")
            else:
                log_info(f"LR finder failed: {e}")
                effective_lr = training_args.learning_rate
                best_state_dict = None
                best_optimizer_state = None
                log_info(f"Using default LR: {effective_lr:.2e}")
        ...
        log_info(f"LR finder completed. Batch size remains {original_batch_size} for training")
    else:
        effective_lr = training_args.learning_rate
        best_state_dict = None
        best_optimizer_state = None
    ...
    log_info(f"Updated training_args.learning_rate={effective_lr:.2e} for training")
    log_info("="*60)
    log_info("FINAL TRAINING CONFIGURATION")
    log_info("="*60)
    ...
    dynamic_accum = DynamicGradientAccumulationCallback(
        trainer=trainer,
        min_accumulation_steps=1,
        max_accumulation_steps=8,
        initial_accumulation_steps=None,
        increase_threshold=0.4,
        decrease_threshold=0.15,
        adjustment_window=75,
        adjustment_cooldown=150,
        log_frequency=25,
        startup_delay_steps=50
    )
    trainer.add_callback(dynamic_accum)
    adaptive_gradient = AdaptiveGradientCallback(
        ...
        norm_factor=1.0,
        per_layer=False,
        memorization_threshold=1.3,
        max_noise_scale=0.06,
        monitor_window_size=50,
        base_noise_scale=0.01,
    )
    trainer.add_callback(adaptive_gradient)
    if training_args.use_lora:
        dropout_callback = CoordinatedDropoutCallback(trainer)
    ...
    if original_train_size <= 10000:
        initial_cycle_length = 200
    elif original_train_size <= 30000:
        initial_cycle_length = 350
    else:
        initial_cycle_length = 500
    

    if total_steps_per_epoch > 0:
        min_reasonable_cycle = max(100, total_steps_per_epoch * 2)
        initial_cycle_length = max(initial_cycle_length, min_reasonable_cycle)
    
    if hasattr(training_args, 'learning_rate') and training_args.learning_rate:
        effective_lr = training_args.learning_rate
    else:
        effective_lr = 5e-5
    
    cycle_callback = AdaptiveCycleCosineLRCallback(
        trainer=trainer,
        output_dir=training_args.output_dir,
        ...
        sharpness_aware_factor=0.05,
        existing_checkpoint=checkpoint_manager,
        enable_hub_push=False
    )
    trainer.add_callback(cycle_callback)
