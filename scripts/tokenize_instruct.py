...
def _normalize_instruction_field(field_name: str, dataset_sample: dict = None) -> str:
    """
    Simple handling for instruct vs instruction field names.
    """
    if dataset_sample is None:
        return field_name
        
    # If specified field exists, use it
    if field_name in dataset_sample:
        return field_name
    
    # Auto-correct common variations
    if field_name == "instruct" and "instruction" in dataset_sample:
        print(f"Auto-correcting field name: 'instruct' -> 'instruction'")
        return "instruction"
    elif field_name == "instruction" and "instruct" in dataset_sample:
        print(f"Auto-correcting field name: 'instruction' -> 'instruct'")
        return "instruct"
    
    return field_name


def _process_custom_dataset_fields(custom_type_dict: dict, dataset_sample: dict = None) -> dict:
    if not custom_type_dict.get("field_output"):
        field_instruction = custom_type_dict.get("field_instruction")
        if field_instruction and dataset_sample:
            field_instruction = _normalize_instruction_field(field_instruction, dataset_sample)
        
        return {
            "type": "completion",
            "field": field_instruction,
        }

    processed_dict = custom_type_dict.copy()
    
    # Normalize instruction field if needed
    if "field_instruction" in processed_dict and dataset_sample:
        processed_dict["field_instruction"] = _normalize_instruction_field(
            processed_dict["field_instruction"], dataset_sample
        )
