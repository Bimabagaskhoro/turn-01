...
def main():
    ...
    log_info(f"DPO Default/Initial learning_rate: {training_args.learning_rate:.2e}")
    log_info(f"DPO LR Finder Config: min_lr={min_lr:.2e}, max_lr={max_lr:.2e}, find_lr={find_lr}")
    
    if find_lr:
        original_batch_size = training_args.per_device_train_batch_size
        lr_finder_batch_size = max(1, original_batch_size // 4)
        
        log_info(f"DPO: Using reduced batch size for LR finder: {original_batch_size} -> {lr_finder_batch_size}")
        
        from copy import deepcopy
        lr_finder_args = deepcopy(training_args)
        lr_finder_args.per_device_train_batch_size = lr_finder_batch_size
        
        lr_finder_trainer = DPOTrainer(
            model=model,
            ref_model=ref_model,
            args=lr_finder_args,
            train_dataset=train_ds,
        ...
        try:
            ...
            log_info(f"DPO Found optimal LR: {effective_lr:.2e}")
        except Exception as e:
            if "cuda" in str(e).lower():
                log_info(f"DPO CUDA error with batch_size={lr_finder_batch_size}: {e}")
                log_info("DPO: Retrying LR finder with batch_size=1")
                ...
                try:
                    ...
                    log_info(f"DPO Found optimal LR with batch_size=1: {effective_lr:.2e}")
                except Exception as e2:
                    log_info(f"DPO LR finder failed even with batch_size=1: {e2}")
                    effective_lr = training_args.learning_rate
                    best_state_dict = None
                    best_optimizer_state = None
                    log_info(f"Using default LR for DPO: {effective_lr:.2e}")
            else:
                log_info(f"DPO LR finder failed: {e}")
                effective_lr = training_args.learning_rate
                best_state_dict = None
                best_optimizer_state = None
                log_info(f"Using fallback LR for DPO: {effective_lr:.2e}")
        ...
        log_info(f"DPO LR finder completed. Batch size remains {original_batch_size} for training")
    else:
        effective_lr = training_args.learning_rate
        best_state_dict = None
        best_optimizer_state = None
        log_info(f"Using configured LR for DPO: {effective_lr:.2e} (LR finder disabled)")
                
    training_args.learning_rate = effective_lr
    log_info(f"Updated DPO training_args.learning_rate={effective_lr:.2e} for training")
    log_info("="*60)
    log_info("DPO FINAL TRAINING CONFIGURATION")
    log_info("="*60)
