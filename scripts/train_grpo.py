...
from callbacks import (
    ...
    ElementWiseOptimizerDropoutCallback,
    AdaptiveGradientCallback,
    ModelCheckpoint,
    EMAEvaluationCallback
)
from lr_finder_les_grpo import find_lr_and_continue
...
def main():
    ...
    min_lr = train_request["min_lr"]
    max_lr = train_request["max_lr"]
    find_lr = train_request["find_lr"]
    
    log_info(f"GRPO Default/Initial learning_rate: {training_args.learning_rate:.2e}")
    log_info(f"GRPO LR Finder Config: min_lr={min_lr:.2e}, max_lr={max_lr:.2e}, find_lr={find_lr}")
    
    if find_lr:
        original_batch_size = training_args.per_device_train_batch_size
        original_use_vllm = training_args.use_vllm
        
        lr_finder_batch_size = max(1, original_batch_size // 4)
        
        log_info(f"GRPO: Using reduced batch size for LR finder: {original_batch_size} -> {lr_finder_batch_size}")
        log_info(f"disable vllm for lr finder {original_use_vllm}")
        
        from copy import deepcopy
        lr_finder_args = deepcopy(training_args)
        lr_finder_args.per_device_train_batch_size = lr_finder_batch_size
        lr_finder_args.use_vllm = False
        
        log_info("GRPO: Creating LR finder trainer...")
        lr_finder_trainer = GRPOTrainer(
            model=model,
        ...
        trainable_params = sum(p.numel() for p in lr_finder_trainer.model.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in lr_finder_trainer.model.parameters())
        log_info(f"GRPO LR Finder: Trainable params: {trainable_params:,} / {total_params:,} ({100*trainable_params/total_params:.2f}%)")
        trainable_param_names = [name for name, p in lr_finder_trainer.model.named_parameters() if p.requires_grad]
        log_info(f"GRPO LR Finder: Trainable parameter count: {len(trainable_param_names)}")
        if len(trainable_param_names) < 50:
            log_info(f"GRPO LR Finder: Trainable params: {trainable_param_names[:10]}...")
        
        try:
            effective_lr, best_state_dict, best_optimizer_state, _ = find_lr_and_continue(
                trainer=lr_finder_trainer,
                start_lr=min_lr,
                end_lr=max_lr,
                preserve_state=True,
                lora=bool(peft_config)
            )
            log_info(f"GRPO Found optimal LR: {effective_lr:.2e}")
        except Exception as e:
            if "cuda" in str(e).lower():
                log_info(f"GRPO CUDA error with batch_size={lr_finder_batch_size}: {e}")
                log_info("GRPO: Retrying LR finder with batch_size=1")
                ...
                try:
                    ...
                    log_info(f"GRPO Found optimal LR with batch_size=1: {effective_lr:.2e}")
                except Exception as e2:
                    log_info(f"GRPO LR finder failed even with batch_size=1: {e2}")
                    effective_lr = training_args.learning_rate
                    best_state_dict = None
                    best_optimizer_state = None
                    log_info(f"Using default LR for GRPO: {effective_lr:.2e}")
            else:
                log_info(f"GRPO LR finder failed: {e}")
                effective_lr = training_args.learning_rate
                best_state_dict = None
                best_optimizer_state = None
                log_info(f"Using fallback LR for GRPO: {effective_lr:.2e}")
        
        torch.cuda.synchronize()

        if hasattr(lr_finder_trainer, 'model'):
            del lr_finder_trainer.model
        if hasattr(lr_finder_trainer, 'optimizer'):
            del lr_finder_trainer.optimizer
        del lr_finder_trainer
        del lr_finder_args
        ...
        if torch.cuda.is_available():
            ...
            log_info(f"GRPO: After LR finder cleanup - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")

        log_info(f"GRPO LR finder completed. Batch size remains {original_batch_size} for training")
        log_info(f"GRPO LR finder completed. use vllm remains {original_use_vllm} for training")
        log_info(f"GRPO LR finder completed")
        
        training_args.per_device_train_batch_size = original_batch_size
        training_args.use_vllm = original_use_vllm
    else:
        effective_lr = training_args.learning_rate
        best_state_dict = None
        best_optimizer_state = None
        log_info(f"Using configured LR for GRPO: {effective_lr:.2e} (LR finder disabled)")

    training_args.learning_rate = effective_lr
    log_info(f"Updated GRPO training_args.learning_rate={effective_lr:.2e} for training")
    log_info(f"GRPO Main trainer will use: batch_size={training_args.per_device_train_batch_size}, use_vllm={training_args.use_vllm}")
    ...
    checkpoint_manager = ModelCheckpoint(
        ...
        max_retries=2,
        lora=bool(peft_config),
        enable_hub_push=False
    )
    
    log_info(f"GRPO: Creating main trainer with optimal LR={training_args.learning_rate:.2e}")
    trainer = GRPOTrainer(
        model=model,
        reward_funcs=wrapped_reward_funcs,
        args=training_args,
    ...
    log_info("="*60)
    log_info("GRPO FINAL TRAINING CONFIGURATION")
    log_info("="*60)
    log_info(f"training_args: {training_args}")
    log_info("="*60)
    
    checkpoint_manager.trainer = trainer
    
    element_dropout = ElementWiseOptimizerDropoutCallback(
        dropout_prob=0.05,
        exclude_bias=True,
        log_frequency=100
    )
    trainer.add_callback(element_dropout)
    adaptive_gradient = AdaptiveGradientCallback(
    ...
    trainer.add_callback(adaptive_gradient)

    ema_callback = EMAEvaluationCallback(
        output_dir=training_args.output_dir,
